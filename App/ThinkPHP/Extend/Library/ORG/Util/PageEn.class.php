...
class Page {
    ...
    protected $coolPages   ;
    // 分页显示定制
	public $config  =    array('header'=>'records','prev'=>'prev','next'=>'next','first'=>'first page','last'=>'last page','theme'=>' %totalRow% %header% %nowPage%/%totalPage% pages %upPage% %downPage% %first%  %prePage%  %linkPage%  %nextPage% %end%');
    // 默认分页变量名
    protected $varPage;
		...
		if(c('cfg_runmode')==0){  //动态分页
			...
			$p    =   $this->varPage;
				...
				if($nowCoolPage == 1){
					...
					$prePage    =   "<a href='".str_replace('__PAGE__',$preRow,$url)."' ><span>prev ".$this->rollPage." pages</span></a>";
				...
				if($nowCoolPage == $this->coolPages){
					...
					$nextPage   =   "<a href='".str_replace('__PAGE__',$nextRow,$url)."' ><span>next ".$this->rollPage." pages</span></a>";
