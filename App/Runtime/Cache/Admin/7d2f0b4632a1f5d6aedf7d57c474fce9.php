...
<div class="add-tab">
	...
	<div class="at_tab">
		<a href="javascript:;" class="cur"><i>留言信息</i></a> 
	</div>
...
<div class="table-list ">
	<table cellpadding="0" cellspacing="0" border="0" width="100%" class="table-list_tb">
		<tr>
			<th width="20"><input type="checkbox" value="" id="check_box" ></th>
			<th width="35">ID</th>
			<th width="120">姓名</th>
			<th width="120">电话</th>
			<th width="150">邮箱</th>
		...
		<?php if($list): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$foo): $mod = ($i % 2 );++$i;?><tr>
			<td align="center"><input type="checkbox" value="<?php echo ($foo['id']); ?>" name="id[]"></td>
			<td align="center"><?php echo ($foo["id"]); ?></td>
			<td  align="center" ><?php echo (escapeshow($foo["username"])); ?></td>
			<td align="center"><?php echo (escapeshow($foo["tel"])); ?></td>
			<td align="center"><?php echo (escapeshow($foo["email"])); ?></td>
			<td align="center"><?php echo (escapeshow($foo["content"])); ?></td>
			<td align="center"><?php if($foo["checkinfo"] == '0'): ?>已审核<?php else: ?><span style="color:red;">未审核</span><?php endif; ?> | <?php if(trim($foo['re_content']) != ''): ?>已回复<?php else: ?><span style="color:red;">未回复</span><?php endif; ?> | <a href="<?php echo U('edit',array('id'=>$foo['id']));?>">查看</a> | <a href="<?php echo U('del',array('id'=>$foo['id']));?>" onclick="return del(0);">删除</a></td>
